...
__constant__ float PI = 3.141592653589793238462;
...
// normalize
inline __host__ __device__ float3 normalize(float3 v) { float invLen = 1.0f/sqrtf(dot(v, v)); return v * invLen; }
// floor
...
inline __host__ __device__ float3 rotate_y(float3 v, float angle) 
{
	float cs = cosf(angle);
	float sn = sinf(angle);
	return {v.x*cs-v.z*sn,v.y,v.z*cs+v.x*sn};
}